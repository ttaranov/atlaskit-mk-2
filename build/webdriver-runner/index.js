...
function getJestExitCode(result) {
  return !result || result.success ? 0 : 1;
}

async function run(testPaths) {
...
function rerunFailedTests(result) {
  return new Promise(async resolve => {
    ...
    if (!failingTestPaths.length) {
      resolve(getJestExitCode(result));
      return;
    }
    ...
    const results = await run(failingTestPaths);
    resolve(getJestExitCode(results));
  });
}
