...
function getUpdateDetails (dependent, changeset, dependents, allWorkSpaces) {
  ...
  //
  const skipPrompt = dependent.type === 'none' && allDependentsFinalised && mustUpdateBecause.length === 0
  return { allDependentsFinalised, mustUpdateBecause, mayUpdateBecause, skipPrompt }
}

async function promptBumptype ({ mustUpdateBecause, mayUpdateBecause, dependentName, updateOptions, isSubsequent }) {
  const mustUpdateList = chalk.green(mustUpdateBecause.join(', '));
  const mayUpdateList = chalk.green(mayUpdateBecause.join(', '));

  if (isSubsequent) logger.log(`Due to other changes you have selected, you need to change the type of release for ${dependentName}`);
...
async function promptAndAssembleReleaseTypes (dependents/*: Array<dependentType> */, changeset/*: changesetType */, cwd/*: string */) {
  ...
  while (dependents.find(dependent => !dependent.finalised)) {
    for (const dependent of dependents) {
      const {
        allDependentsFinalised, mustUpdateBecause, mayUpdateBecause, skipPrompt
      ...
      const bumpType/*: bumpType */ =  await promptBumptype({
          mustUpdateBecause, mayUpdateBecause, dependentName, updateOptions, isSubsequent: !!dependent.type
      })
