...
const getImportFilepath = (currentFilepath, relativeImportPath) => {
  const importPath = path.join(path.parse(currentFilepath).dir, relativeImportPath);
...
const followRelativeImports = (filepath) => {
  ...
  traverse(ast, {
    enter(nodePath) {
      if (
        ...
        const importedFilepath = getImportFilepath(filepath, nodePath.node.source.value);
        // Recursion!
  ...
  // De-dupe imports
  return imports.reduce((dedupedImports, file) => (
    dedupedImports.includes(file) ? dedupedImports : [...dedupedImports, file]
  ), []);
};
...
module.exports = (component /*: string */, example /*: string */) => {
  // The component root
  const root = path.join(process.cwd(), '..', 'packages', 'elements', component);
  ...
  const destinationDir = path.join(process.cwd(), 'dist', 'sandbox', component, example);
  ...
  srcFiles.forEach((filepath) => {
    const destinationPath = path.join(destinationSrcDir, filepath.replace(root, ''));
...
`;
  ...
  const componentPackageJSON = JSON.parse(fs.readFileSync(componentPackageJSONPath));
  ...
  const sandboxFiles = globby.sync(path.join(destinationDir, '**/*.*'))
