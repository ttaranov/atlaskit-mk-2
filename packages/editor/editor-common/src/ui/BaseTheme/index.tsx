...
import { akEditorSize } from '../../styles';
import { WidthConsumer } from '../WidthProvider';

export function getSizes(width: number = 0) {
  if (width >= 1060 && width < 1320) {
    return { baseFont: fontSize() + 2, size: akEditorSize.M }; // 16px
  } else if (width >= 1320) {
    return { baseFont: fontSize() + 4, size: akEditorSize.L }; // 18px
  }
  return { baseFont: fontSize(), size: akEditorSize.S }; // 14px
}
