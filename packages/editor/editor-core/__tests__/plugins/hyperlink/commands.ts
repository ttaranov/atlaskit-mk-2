...
describe('hyperlink commands', () => {
  describe('#setLinkHref', () => {
    it('should not set the link href when pos is not inside a link node', () => {
      ...
      expect(
        setLinkHref(sel, 'https://google.com')(view.state, view.dispatch, view),
      ).toBe(false);
    });
    it('should remove the link mark when the href is an empty string', () => {
      ...
      expect(setLinkHref(sel, '')(view.state, view.dispatch, view)).toBe(true);
    ...
    it('should set normalized link href when the href is non-empty', () => {
      ...
      expect(
        setLinkHref(sel, 'google.com')(view.state, view.dispatch, view),
      ).toBe(true);
    ...
    it('should set mailto: prefix when the href is email-like', () => {
      ...
      expect(
        setLinkHref(sel, 'scott@google.com')(view.state, view.dispatch, view),
      ).toBe(true);
      expect(view.state.doc).toEqualDocument(
  ...
  describe('#setLinkText', () => {
    it('should not set the link text when pos is not at a link node', () => {
      const { editorView: view, sel } = editor(doc(p('{<>}')));
      expect(setLinkText(sel, 'google')(view.state, view.dispatch, view)).toBe(
        false,
      );
    });
    it('should not set the link text when text is an empty string', () => {
      ...
      expect(setLinkText(sel, '')(view.state, view.dispatch, view)).toBe(false);
    });
    it('should not set the link text when text is equal to the node.text', () => {
      ...
      expect(
        setLinkText(sel, 'google.com')(view.state, view.dispatch, view),
      ).toBe(false);
    });
    it('should set the link text when the text is non-empty', () => {
      ...
      expect(setLinkText(sel, 'hi!')(view.state, view.dispatch, view)).toBe(
        true,
      );
      expect(view.state.doc).toEqualDocument(
  ...
  describe('#insertLink', () => {
    it('should not insert link when selection is inside an incompatible node', () => {
      ...
      expect(
        insertLink(sel, sel, 'google.com')(view.state, view.dispatch, view),
      ).toBe(false);
    });
    it('should not insert link when selection is across incompatible nodes', () => {
      ...
      expect(
        insertLink(from, to, 'google.com')(view.state, view.dispatch, view),
      ).toBe(false);
    });
    ...
    it('should not insert link when href is an empty string', () => {
      const { editorView: view, sel } = editor(doc(p('{<>}')));
      expect(insertLink(sel, sel, '')(view.state, view.dispatch, view)).toBe(
        false,
      );
    });
    ...
    it('should set mailto: prefix when the href is email-like', () => {
      const { editorView: view, sel } = editor(doc(p('{<>}')));
      expect(
        insertLink(sel, sel, 'scott@google.com')(
          view.state,
          view.dispatch,
          view,
        ),
    ...
    it('should ensure the editor is focused after inserting a link', () => {
      const {
        editorView: view,
        refs: { '<': from, '>': to },
      } = editor(doc(p('{<}example_link{>}')));
      const focusMock = jest.spyOn(view, 'focus');
      expect(
        insertLink(from, to, 'google.com')(view.state, view.dispatch, view),
      ).toBe(true);
  ...
  describe('#removeLink', () => {
    it('should remove the link mark when the href is an empty string', () => {
      ...
      expect(removeLink(sel)(view.state, view.dispatch, view)).toBe(true);
      expect(view.state.doc).toEqualDocument(doc(p('text')));
    });
    it('should not set remove the link href when pos is not inside a link node', () => {
      const { editorView: view, sel } = editor(doc(p('{<>}')));
      expect(removeLink(sel)(view.state, view.dispatch, view)).toBe(false);
    });
  });
  describe('#showLinkToolbar', () => {
    it('should trigger the SHOW_INSERT_TOOLBAR for the hyperlink plugin', () => {
      ...
      expect(showLinkToolbar()(view.state, view.dispatch, view)).toBe(true);
  ...
  describe('#hideLinkToolbar', () => {
    it('should trigger the HIDE_TOOLBAR for the hyperlink plugin', () => {
      const { editorView: view } = editor(doc(p('{<>}')));
      const dispatchMock = jest.spyOn(view, 'dispatch');
      expect(hideLinkToolbar()(view.state, view.dispatch, view)).toBe(true);
      expect(dispatchMock.mock.calls[0][0].getMeta(hyperlinkStateKey)).toBe(
        LinkAction.HIDE_TOOLBAR,
      );
    });

    it('should focus the editor after the HIDE_TOOLBAR command dispatched', () => {
      const { editorView: view } = editor(doc(p('{<>}')));
      const focusMock = jest.spyOn(view, 'focus');
      expect(hideLinkToolbar()(view.state, view.dispatch, view)).toBe(true);
      expect(focusMock).toHaveBeenCalled();
