...
const getTextFormattingState = (editorState: EditorState): TextFormattingState => {
  ...
  if (subsup) {
    ...
    state.subscriptDisabled = state.codeActive ? true : !toggleMark(subsup, { type: 'sub' })(
      editorState,
    );
    state.superscriptActive = markActive(editorState, supMark);;
    state.superscriptDisabled = state.codeActive
    ? true
    : !toggleMark(subsup, { type: 'sup' })(
      editorState,
    );
  }
  if (underline) {
