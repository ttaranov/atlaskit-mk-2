...
export interface Props {
  ...
  isOpen: boolean;
  selectionRect: { top: number; bottom: number; left: number; right: number };
...
export default class ContextualMenu extends Component<Props, State> {
  ...
  private onMenuItemActivated = ({ item }) => {
    ...
    switch (item.value.name) {
      ...
      case 'delete_column':
        ...
        deleteColumn(selectionRect.left)(state, dispatch);
        this.toggleOpen();
        break;
      case 'delete_row':
        analytics.trackEvent('atlassian.editor.format.table.delete_row.button');
        deleteRow(selectionRect.top)(state, dispatch);
        this.toggleOpen();
        break;
    }
  };
  ...
  private handleItemMouseEnter = ({ item }) => {
    ...
    if (item.value.name === 'delete_column') {
      ...
      hoverColumns(columnIndexes, true)(state, dispatch);
    }
    if (item.value.name === 'delete_row') {
      ...
      hoverRows(rowIndexes, true)(state, dispatch);
    }
  };
  ...
  private setColor = color => {
    const { state, dispatch } = this.props.editorView;
    setCellsAttrs({ background: color })(state, dispatch);
    this.toggleOpen();
  };
}
