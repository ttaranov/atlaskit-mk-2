...
describe(`${name} - deprecated API - item`, () => {
  ...
  describe('rendering', () => {
    ...
    it('should render descriptions after the content', () => {
      expect(mount(<Item description="test">sample</Item>).text()).toBe(
        'sampletest',
      );
    });
    ...
    it('should render elemBefore for all items', () => {
      ...
      expect(mount(<Item elemBefore={Icon}>sample</Item>).contains(Icon)).toBe(
        true,
      );
      expect(
        mount(
          <Item elemBefore={Icon} type="checkbox">
            sample
          </Item>,
        ).contains(Icon),
      ).toBe(true);
      expect(
        mount(
          <Item elemBefore={Icon} type="radio">
            sample
          </Item>,
        ).contains(Icon),
      ).toBe(true);
    ...
    it('should render elemAfter for all items', () => {
      const Icon = <div id="icon">icon</div>;
      expect(mount(<Item elemAfter={Icon}>sample</Item>).contains(Icon)).toBe(
        true,
      );
      expect(
        mount(
          <Item elemAfter={Icon} type="checkbox">
            sample
          </Item>,
        ).contains(Icon),
      ).toBe(true);
      expect(
        mount(
          <Item elemAfter={Icon} type="radio">
            sample
          </Item>,
        ).contains(Icon),
      ).toBe(true);
    ...
    it('should render icon for the radio or checkbox element', () => {
      expect(mount(<Item type="radio" />).find(Radio).length).toBeGreaterThan(
        0,
      );
      expect(
        mount(<Item type="checkbox" />).find(Checkbox).length,
      ).toBeGreaterThan(0);
    });
    ...
    it('should render tooltip when tooltipDescription is not empty', () => {
      expect(
        mount(<Item tooltipDescription="foo" />).find(Tooltip).length,
      ).toBe(1);
    });

    it('should NOT render tooltip when tooltipDescription is empty', () => {
      expect(mount(<Item />).find(Tooltip).length).toBe(0);
      expect(mount(<Item tooltipPosition="left" />).find(Tooltip).length).toBe(
        0,
      );
    });
  });
  ...
  describe('variants', () => {
    it('should render the correct "type" of styled component', () => {
      expect(
        mount(<Item type="link" href="https://atlassian.design" />)
          .find(Anchor)
          .exists(),
      ).toBe(true);
      expect(
        mount(<Item type="checkbox" />)
          .find(Span)
          .exists(),
      ).toBe(true);
      expect(
        mount(<Item type="radio" />)
          .find(Span)
          .exists(),
      ).toBe(true);
    });
    ...
    it('should have "disabled" data-attribute when disabled', () => {
      expect(
        mount(<Item type="link" isDisabled />)
          .find(Span)
          .prop('data-test-disabled'),
      ).toBe(true);
      expect(
        mount(<Item type="radio" isDisabled />)
          .find(Span)
          .prop('data-test-disabled'),
      ).toBe(true);
      expect(
        mount(<Item type="checkbox" isDisabled />)
          .find(Span)
          .prop('data-test-disabled'),
      ).toBe(true);
    });
    ...
    it('should have "active" data-attribute when link item is active', () => {
      expect(
        mount(<Item type="link" isActive />)
          .find(Span)
          .prop('data-test-active'),
      ).toBe(true);
    });

    it('should have "selected" data-attribute when option item is selected', () => {
      expect(
        mount(<Item type="option" isSelected />)
          .find(Span)
          .prop('data-test-selected'),
      ).toBe(true);
    });

    it('should NOT have "active" prop passed to Element for any other item types', () => {
      expect(
        mount(<Item type="radio" isActive />)
          .find(Element)
          .prop('isActive'),
      ).toBe(false);
      expect(
        mount(<Item type="checkbox" isActive />)
          .find(Element)
          .prop('isActive'),
      ).toBe(false);
    });
    ...
    it('should have "checked" data-attribute when checkbox or radio is checked', () => {
      expect(
        mount(<Item type="checkbox" isChecked />)
          .find(Span)
          .prop('data-test-checked'),
      ).toBe(true);
      expect(
        mount(<Item type="radio" isChecked />)
          .find(Span)
          .prop('data-test-checked'),
      ).toBe(true);
    });
    ...
    it('should NOT have "checked" class for any other items', () => {
      expect(
        mount(<Item type="link" isChecked />)
          .find(Span)
          .prop('data-test-checked'),
      ).toBe(false);
    });
    ...
    it('should have "hidden" data-attribute when item is hidden', () => {
      expect(
        mount(<Item type="link" isHidden />)
          .find(Span)
          .prop('data-test-hidden'),
      ).toBe(true);
      expect(
        mount(<Item type="checkbox" isHidden />)
          .find(Span)
          .prop('data-test-hidden'),
      ).toBe(true);
      expect(
        mount(<Item type="radio" isHidden />)
          .find(Span)
          .prop('data-test-hidden'),
      ).toBe(true);
    });
    ...
    describe('onActivate', () => {
      ...
      it('should not be activated when disabled', () => {
        const disabledWrapper = mount(
          <Item onActivate={onActivate} isDisabled />,
        ).find(Item);
  ...
  describe('secondary text', () => {
    it('should render content inside', () => {
      expect(mount(<SecondaryText>secondary text</SecondaryText>).text()).toBe(
        'secondary text',
      );
    });
  });
