...
function isInteger(props, propName, componentName) {
  ...
  if (!Number.isInteger(props[propName]) && props[propName] !== Infinity) {
    const message = `'${propName}' should be an integer or an infinity, got ${
      props[propName]
    } instead`;
...
function isIn01Range(props, propName, componentName) {
  ...
  if (propValue !== Number(propValue) || propValue < 0 || propValue > 1) {
    const message = `'${propName}' should be a number in the range [0,1], got ${
      propValue
    } instead`;
    return createError(propName, componentName, message);
  }
...
function rowCells(props, propName, componentName, ...rest) {
  const initialCheck = PropTypes.arrayOf(rowCell)(
    props,
    propName,
    componentName,
    ...rest,
  ...
  const isAllRowsSameLength = props[propName].reduce(
    (result, row) => result && row.length === props.head.cells.length,
    true,
  );
  if (!isAllRowsSameLength) {
    return createError(
      propName,
      componentName,
      'One of the rows has different cell-count compared to head.',
    );
  }
  return null;
}
...
function isSortable(props, propName, componentName, ...rest) {
  ...
  if (props[propName] && !props.key) {
    return createError(
      propName,
      componentName,
      `'${propName}' can't be set to true, if the 'key' prop is missing.`,
    );
  }
  return null;
}
...
function sortKey(props, propName, componentName) {
  ...
  if (!headHasKey) {
    return createError(
      propName,
      componentName,
      `Cell with ${props[propName]} key not found in head.`,
    );
  }
  return null;
}
