...
describe('rankable/Body', () => {
  ...
  describe('onDragStart', () => {
    it('onRankStart is called with proper arguments', () => {
      ...
      const wrapper = shallow(<RankableBody {...defaultProps} isRanking />);
  ...
  describe('onDragEnd', () => {
    ...
    it('onRankEnd is called with proper empty destination if drag was cancelled', () => {
      ...
      const wrapper = shallow(<RankableBody {...defaultProps} />);

      const dndContext = wrapper.find(DragDropContext);
      dndContext.simulate(
        'dragEnd',
        createDragEndProps(sourceKey, sourceIndex),
      );
