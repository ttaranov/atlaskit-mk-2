...
describe(name, () => {
  describe('stateless', () => {
    ...
    it('should invoke callback passed to onSetPage', () => {
      ...
      const wrapper = mount(
        <PaginationStateless total={3} current={2} onSetPage={onSetPage} />,
      );
      const buttons = wrapper.find(Button);
    ...
    describe("shouldn't invoke callback passed to onSetPage", () => {
      ...
      it('when clicked on Prev and first page is active', () => {
        const onSetPage = jest.fn();
        const wrapper = mount(
          <PaginationStateless total={3} current={1} onSetPage={onSetPage} />,
        );
        const buttons = wrapper.find(Button);
        buttons.at(1).simulate('click');
        expect(onSetPage).not.toHaveBeenCalled();
      ...
      it('when clicked on Next and last page is active', () => {
        const onSetPage = jest.fn();
        const wrapper = mount(
          <PaginationStateless total={3} current={3} onSetPage={onSetPage} />,
        );
        const buttons = wrapper.find(Button);
