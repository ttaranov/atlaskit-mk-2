...
export default class AbstractReactionsResource implements ReactionsProvider {
  ...
  toggleReaction(containerAri: string, ari: string, emojiId: string) {
    ...
    const hasReaction =
      this.cachedReactions[key] &&
      this.cachedReactions[key].filter(r => equalEmojiId(r.emojiId, emojiId));
    const hasReacted =
      hasReaction && hasReaction.length !== 0 && hasReaction[0].reacted;
  ...
  subscribe(subscriptionKey: ObjectReactionKey, handler: Function): void {
    ...
    debounced = setTimeout(() => {
      this.getReactions(this.batchedKeys).then(reactions => {
...
export class ReactionsResource extends AbstractReactionsResource
