...
* \`selectedValue\`: This is an optional prop that is compared against the value of each passed in option. The matching option will be instantiated as a Radio component with isChecked set to true. If this prop is left undefined, the selection of any given radio is managed internally within the state of the RadioGroup component.

* \`defaultSelectedValue\`: This is an optional prop that is set as the selectedValue in state initially. Further interactions with radio options rendered by this RadioGroup will override this value with the internally managed selected state.

\*\`onRadioChange\` in field-radio-group is now \`onChange\` in @atlaskit/radio.

Additionally, while \`defaultSelected\` use to be a valid property within the passed in items array in \`field-radio-group\`; this is no longer the case in \`@atlaskit/radio\`, as the responsibility of this property is now passed up to the defaultSelectedValue prop on the RadioGroup component.

## Radio

The \`isSelected\` prop in field-radio-group has been replaced with the \`isChecked\` prop in @atlaskit/radio, to inline it with conventions established both in native radio elements and within the context of React.
