...
import { DropdownMenuStatelessWithoutAnalytics as StatelessMenu } from '../src/components/DropdownMenuStateless';
...
describe('dropdown menu', () => {
  ...
  describe('render', () => {
    ...
    it('should pass required properties to the button trigger', () => {
      [
        <Menu items={itemsList} triggerType="button">
          text
        </Menu>,
    ...
    it('should pass elemAfter to Group', () => {
      const menu = mount(
        <Menu items={itemsList} defaultOpen>
          text
        </Menu>,
      );
  ...
  describe('show/hide logic', () => {
    ...
    it('interacting with link item should close the dropdown', () => {
      ...
      const item = wrapper.find('[role="menuitem"]').at(1);
      expect(wrapper.state().isOpen).toBe(true);
      item.simulate('click');
      expect(wrapper.state().isOpen).toBe(false);
    });
    ...
    it('interacting with checkbox item should not close the menu', () => {
      const items = [
        {
          heading: 'group',
          items: [{ content: 'item 1', type: 'checkbox' }],
        },
      ];
      const wrapper = mount(
        <Menu items={items} defaultOpen>
      ...
      const item = wrapper.find('[role="menuitemcheckbox"]').at(1);
      expect(wrapper.state().isOpen).toBe(true);
      item.simulate('click');
      expect(wrapper.state().isOpen).toBe(true);
    });
    ...
    it('interacting with radio item should not close the menu', () => {
      const items = [
        {
          heading: 'group',
          items: [{ content: 'item 1', type: 'radio' }],
        },
      ];
      const wrapper = mount(
        <Menu items={items} defaultOpen>
      ...
      const item = wrapper.find('[role="menuitemradio"]').at(1);
      expect(wrapper.state().isOpen).toBe(true);
      item.simulate('click');
      expect(wrapper.state().isOpen).toBe(true);
    });
  ...
  describe('onItemActivated', () => {
    ...
    it('should pass the item when activated', () => {
      ...
      const attrsItem = attrs && attrs.item;
      expect(attrsItem).not.toBe(undefined);
      expect(attrsItem).toBe(items[0].items[0]);
      expect(attrsItem).toEqual({
        content: 'item 1',
        type: 'checkbox',
        isChecked: true,
      });
        ...
        const Items = [
          {
            ...
            items: [
              { content: '0' },
              { content: '1' },
              { content: '2', isHidden: true },
              { content: '3', isHidden: true },
              { content: '4' },
            ],
          },
        ];
        ...
        const Items = [
          {
            heading: 'group',
            items: [
              { content: '0', isHidden: true },
              { content: '1', isHidden: true },
              { content: '2' },
            ],
          },
        ];
