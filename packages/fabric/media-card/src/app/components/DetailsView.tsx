...
export class DetailsView extends React.Component<MetaViewProps, {}> {
  renderWidget(data: AppCardDetails) {
    ...
    if (title) {
      attrs.push(
        <Title key="title" inverse={isInversed}>
          {title}:
        </Title>,
      );
    }
    ...
    if (users) {
      attrs.push(
        <AvatarStack
          ...
          max={4}
          avatars={users.map(({ icon: { url, label } }) => ({
            src: url,
            label,
          }))}
        />,
    ...
    if (lozenge) {
      attrs.push(
        <Lozenge key="lozenge" appearance={lozenge.appearance || 'default'}>
          {lozenge.text}
        </Lozenge>,
      );
    }
    ...
    if (attrs.length === 0) {
      ...
      console.warn(
        `SmartCardView: A metadata item doesn't contain any supported attributes: ${JSON.stringify(
          attrs,
          null,
          2,
        )}`,
    ...
    return (
      <Wrapper contentMaxWidth={contentMaxWidth}>
        {meta.map((data, index) => (
          <Widget key={index}>
            {' '}
            {/* no unique ID so can't avoid using the index for the key here */}
