...
import { MediaFile, validateMediaFile } from '../domain/file';
...
export class UploadService {
  ...
  private generateTarget = (rawParams: Array<string>): string => {
    ...
    return `${this.uploadChunkUrl}/${hash}-${resumableCurrentChunkSize}?${
      authQueryParameters
    }`;
  };
      ...
      () => {
        ...
        const files = resumableFiles.map(resumableFile => {
          ...
          if (file.type.match(/^image\//)) {
            getImagePreview(resumableFile.file).then(preview => {
  ...
  private finalizeFile(resumableFile: ResumableFile): void {
    ...
    this.api
      ...
      .then(fileId => this.processFile(mediaClient, fileId, resumableFile))
      .catch(error => {
  ...
  private processFile(
    ...
    return this.fetchFileMetadata(mediaClient, fileId, resumableFile)
      .then(metadata => {
        const file = this.mapResumableFileToMediaFile(
          resumableFile,
          metadata.id,
        );
        const progress = new SmartMediaProgress(
  ...
  private fetchFileMetadata(
    ...
    if (fetchMetadata) {
      this.emit('file-converting', {
        file: this.mapResumableFileToMediaFile(resumableFile, publicId),
      });
