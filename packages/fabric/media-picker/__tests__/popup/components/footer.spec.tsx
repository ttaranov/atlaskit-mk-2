...
import {
  Footer,
  default as ConnectedFooter,
} from '../../../src/popup/components/footer/footer';
import {
  Wrapper,
  CancelButton,
  InsertButton,
} from '../../../src/popup/components/footer/styled';
import { startImport, hidePopup } from '../../../src/popup/actions';
...
describe('<Footer />', () => {
  ...
  describe('#render()', () => {
    ...
    it('should show the insert button when itemCount>0', () => {
      ...
      expect(element.find(InsertButton).props().isDisabled).toBeFalsy();
    });
    ...
    it('should disable the insert button when canInsert=false', () => {
      ...
      expect(element.find(InsertButton).props().isDisabled).toBeTruthy();
    });
    ...
    it('should not pluralize the insert button text when itemCount=1', () => {
      const element = shallow(
        <Footer
          selectedItems={[SELECTED_ITEM]}
          canInsert={true}
      ...
      expect(element.find(InsertButton).prop('children')).not.toMatch('files');
    });
