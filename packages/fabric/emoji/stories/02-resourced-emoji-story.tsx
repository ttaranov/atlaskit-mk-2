...
storiesOf(`${name}/ResourcedEmoji`, module)
  ...
  .add('skin tones', () => (
    <p style={lineStyle()}>
      <span>
        ...
        <ResourcedEmoji
          emojiId={{
            shortName: ':thumbsup::skin-tone-7:',
            id: '1f44d-1f3fg' /* invalid - will fallback to text render */,
          }}
          emojiProvider={getEmojiResource() as Promise<EmojiProvider>}
          showTooltip={true}
        />
  ...
  .add('slow loading emoji', () => {
    ...
    const handleStandardRef = ref => {
      loadStandardRef = ref;
    };
    const handleAtlassianRef = ref => {
      loadAtlassianRef = ref;
    };
    ...
    return (
      <div style={lineStyle()}>
        ...
        <div>
          <button onClick={loadStandard} ref={handleStandardRef}>
            Load Standard Emojis
          </button>
          <button onClick={loadAtlassian} ref={handleAtlassianRef}>
            Load Atlassian Emojis
          </button>
        </div>
      </div>
    );
