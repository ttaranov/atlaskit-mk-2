...
const cleanUpSchema = json => {
  ...
  const extensionContentItems = json.definitions.extension_content.items;
  if (extensionContentItems && extensionContentItems.anyOf) {
  ...
  const listNodeItems = json.definitions.listItem_node.properties.content.items;
  if (listNodeItems && listNodeItems.anyOf) {
