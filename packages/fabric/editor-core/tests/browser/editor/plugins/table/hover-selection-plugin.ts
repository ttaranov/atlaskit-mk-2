...
import {
  createEvent, chaiPlugin, doc, p, makeEditor, table, tr, tdEmpty, tdCursor
} from '@atlaskit/editor-test-helpers';
import { DecorationSet } from 'prosemirror-view';
import { resetHoverSelection, hoverColumn, hoverRow, hoverTable } from '../../../../../src/editor/plugins/table/actions';
import { tableStartPos } from '../../../../../src/editor/plugins/table/utils';
import hoverSelectionPlugin, { pluginKey as hoverPluginKey } from '../../../../../src/editor/plugins/table/hover-selection-plugin';
        ...
        context(`when called with ${column}`, () => {
          it(`it should create a hover selection of ${column} column`, () => {
            const { plugin, pluginState, editorView } = editor(
              doc(p('text'), table(tr(tdCursor, tdEmpty, tdEmpty), tr(tdEmpty, tdEmpty, tdEmpty)))
            );
      ...
      [0, 1, 2].forEach(row => {
        context(`when called with ${row}`, () => {
          it(`it should create a hover selection of ${row} row`, () => {
            const { plugin, pluginState, editorView } = editor(
              doc(p('text'), table(tr(tdCursor, tdEmpty), tr(tdEmpty, tdEmpty), tr(tdEmpty, tdEmpty)))
            );
            plugin.props.onFocus!(editorView, event);
  ...
  describe('hoverTable()', () => {
    context('when table has 3 rows', () => {
      it('it should create a hover selection of the whole table', () => {
        ...
        expect(hoverPluginKey.getState(editorView.state)).to.equal(DecorationSet.empty);
      });
    });
  });
});
