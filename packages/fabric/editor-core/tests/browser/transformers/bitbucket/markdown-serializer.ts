...
describe('BitbucketTransformer: serializer', () => {
  ...
  it('should serialize paragraphs', () => {
    ...
    expect(markdownSerializer.serialize(doc(p('foo'), p('bar')))).to.eq(
      'foo\n\nbar'
    );

    const longText = stringRepeat('foo ', 100);
    expect(markdownSerializer.serialize(doc(p(longText), p(longText)))).to.eq(
      `${longText}\n\n${longText}`
    );
  });

  it('should preserve multiple blank lines using zero-non-width', () => {
    expect(markdownSerializer.serialize(doc(p('foo'), p(), p('bar')))).to.eq(
      'foo\n\n\u200c\n\nbar'
    );
  ...
  it('should escape 4 spaces at the beginning of the line with zero-width-non-joiner', () => {
    ...
    expect(markdownSerializer.serialize(doc(p(`${fourSpaces}hello`)))).to.eq(
      `\u200c${fourSpaces}hello`
    );
  ...
  it('should preserve leading and trailing blank lines using zero-non-width', () => {
    expect(markdownSerializer.serialize(doc(p(), p('bar')))).to.eq(
      '\u200c\n\nbar'
    );

    expect(markdownSerializer.serialize(doc(p(), p(), p('bar')))).to.eq(
      '\u200c\n\n\u200c\n\nbar'
    );

    expect(markdownSerializer.serialize(doc(p('foo'), p()))).to.eq(
      'foo\n\n\u200c'
    );

    expect(markdownSerializer.serialize(doc(p('foo'), p(), p()))).to.eq(
      'foo\n\n\u200c\n\n\u200c'
    );
  });
  ...
  it('should not skip [ & ]', () => {
    expect(markdownSerializer.serialize(doc(p('[hello]')))).to.eq(
      '\\[hello\\]'
    );
  });
  ...
  describe('code block', () => {
    ...
    it('with adjacent code block keeps empty space between', () => {
      expect(markdownSerializer.serialize(doc(pre('foo'), pre('bar')))).to.eq(
        '    foo\n\n    bar'
      );
    });
    ...
    it('with attributes uses backtick notation and preserves attributes', () => {
      ...
      expect(markdownSerializer.serialize(doc(js('foo')))).to.eq(
        '```js\nfoo\n```'
      );
    ...
    it('with no text is preserved', () => {
      expect(markdownSerializer.serialize(doc(pre('')))).to.eq('    \u200c');
    ...
    it('via backticks that includes backticks is properly fenced', () => {
      ...
      expect(markdownSerializer.serialize(doc(css('````')))).to.eq(
        '`````css\n````\n`````',
        'Unmatched backtick fence'
      );

      expect(markdownSerializer.serialize(doc(css('````js')))).to.eq(
        '`````css\n````js\n`````',
  ...
  it('should serialize headings (level 1 - 6)', () => {
    ...
    expect(markdownSerializer.serialize(doc(h2('level 2')))).to.eq(
      '## level 2'
    );
    expect(markdownSerializer.serialize(doc(h3('level 3')))).to.eq(
      '### level 3'
    );
    expect(markdownSerializer.serialize(doc(h4('level 4')))).to.eq(
      '#### level 4'
    );
    expect(markdownSerializer.serialize(doc(h5('level 5')))).to.eq(
      '##### level 5'
    );
    expect(markdownSerializer.serialize(doc(h6('level 6')))).to.eq(
      '###### level 6'
    );
  ...
  it('should serialize horizontal_rule', () => {
    ...
    expect(markdownSerializer.serialize(doc(p('foo'), hr, p('bar')))).to.eq(
      'foo\n\n---\n\nbar'
    );
  });
  ...
  it('should serialize hardBreak to newline', () => {
    expect(markdownSerializer.serialize(doc(p('foo ', br, 'bar')))).to.eq(
      'foo   \nbar'
    );
  });
  ...
  describe('blockquotes', () => {
    it('should serialized', () => {
      expect(markdownSerializer.serialize(doc(blockquote(p('foo'))))).to.eq(
        '> foo'
      );
      expect(
        markdownSerializer.serialize(
  ...
  describe('marks -', () => {
    ...
    it('should ignore emojiQuery mark', () => {
      expect(markdownSerializer.serialize(doc(p(emojiQuery(':grin'))))).to.eq(
        ':grin'
      );
    });
    ...
    it('should serialize strong', () => {
      expect(markdownSerializer.serialize(doc(p(strong('foo'))))).to.eq(
        '**foo**'
      );
      expect(
    ...
    it('should serialize strikethrough', () => {
      expect(markdownSerializer.serialize(doc(p(strike('foo'))))).to.eq(
        '~~foo~~'
      );
      expect(
    ...
    it('should serialize code', () => {
      expect(markdownSerializer.serialize(doc(p(code('foo()'))))).to.eq(
        '`foo`'
      );
      expect(
    ...
    xdescribe('links', () => {
      ...
      it('with title containing double-quotes to serialize', () => {
        ...
        expect(markdownSerializer.serialize(doc(p(link('foo'))))).to.eq(
          '[foo](http://example.com "some " "title"")'
        );
      });
    ...
    describe('emphasis -', () => {
      it('asterisk within strings should be escaped', () => {
        expect(markdownSerializer.serialize(doc(p('*foo bar*')))).to.eq(
          '\\*foo bar\\*'
        );
      ...
      it('underscore within strings should be escaped', () => {
        expect(markdownSerializer.serialize(doc(p('_foo bar_')))).to.eq(
          '\\_foo bar\\_'
        );

        expect(markdownSerializer.serialize(doc(p('__foo bar__')))).to.eq(
          '\\_\\_foo bar\\_\\_'
        );

        expect(markdownSerializer.serialize(doc(p('___foo bar___')))).to.eq(
          '\\_\\_\\_foo bar\\_\\_\\_'
        );
      });

      it('"strong em" should be escaped in its entirety', () => {
        expect(markdownSerializer.serialize(doc(p('***strong**em*')))).to.eq(
          '\\*\\*\\*strong\\*\\*em\\*'
        );
      });

      it('"smart em" should be escaped in its entirety', () => {
        expect(markdownSerializer.serialize(doc(p('_smart_emphasis_')))).to.eq(
          '\\_smart\\_emphasis\\_'
        );
      });
      ...
      it('combinations should be properly serialized', () => {
        ...
        expect(markdownSerializer.serialize(doc(p(code('**bar baz**'))))).to.eq(
          '`**bar baz**`'
        );

        expect(markdownSerializer.serialize(doc(p(code('__bar_baz__'))))).to.eq(
          '`__bar_baz__`'
        );
      });
    });
    ...
    describe('tilde ~', () => {
      it('should not escape tilde ~', () => {
        expect(markdownSerializer.serialize(doc(p('~')))).to.eq('~');
      });
    });
  });
