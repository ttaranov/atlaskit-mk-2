...
import { selectTable } from '../../../src/editor/plugins/table/actions';
import {
  isTableSelected,
  isRowSelected,
  isColumnSelected
...
describe('TableFloatingControls', () => {
  ...
  describe('CornerControls', () => {
    context('when isTableSelected is true', () => {
      it('should render selected header', () => {
        ...
        expect(floatingControls.find(CornerControls).prop('isSelected')(editorView.state)).to.equal(true);
        floatingControls.unmount();
      });
    });
  });
