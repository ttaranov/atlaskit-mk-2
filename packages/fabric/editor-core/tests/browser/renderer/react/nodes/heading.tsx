...
describe('<Heading />', () => {
  ...
  it('should wrap content with <h1>-tag', () => {
    expect(heading1.name()).to.equal('h1');
  });

  it('should wrap content with <h2>-tag', () => {
    expect(heading2.name()).to.equal('h2');
  });

  it('should wrap content with <h3>-tag', () => {
    expect(heading3.name()).to.equal('h3');
  });

  it('should wrap content with <h4>-tag', () => {
    expect(heading4.name()).to.equal('h4');
  });

  it('should wrap content with <h5>-tag', () => {
    expect(heading5.name()).to.equal('h5');
  });

  it('should wrap content with <h6>-tag', () => {
    expect(heading6.name()).to.equal('h6');
  });
});
