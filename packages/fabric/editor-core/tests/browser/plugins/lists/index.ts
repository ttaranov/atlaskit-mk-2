  ...
  describe('keymap', () => {
    ...
    context('when hit Tab', () => {
      it('should call indent analytics event', () => {
        ...
        expect(trackEvent.calledWith('atlassian.editor.format.list.indent.keyboard')).to.equal(
          true
        );
      });
    });
    ...
    context('when hit Shift-Tab', () => {
      it('should call outdent analytics event', () => {
        ...
        expect(trackEvent.calledWith('atlassian.editor.format.list.outdent.keyboard')).to.equal(
          true
        );
      });
    });
      ...
      context('when on a mac', () => {
        context('when hit Cmd-Alt-7', () => {
          it('should toggle ordered list', () => {
            ...
            expect(
              trackEvent.calledWith('atlassian.editor.format.list.numbered.keyboard')
            ).to.equal(true);
          });
        });
        ...
        context('when hit Cmd-Alt-8', () => {
          it('should toggle bullet list', () => {
            ...
            expect(trackEvent.calledWith('atlassian.editor.format.list.bullet.keyboard')).to.equal(
              true
            );
          });
        });
  ...
  describe('API', () => {
    ...
    describe('untoggling a list', () => {
      ...
      it('should allow untoggling part of a list based on selection', () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('One')), li(p('{<}Two')), li(p('Three{>}')), li(p('Four'))))
        );
      ...
      it('should untoggle all list items with different ancestors in selection', () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('One')), li(p('{<}Two')), li(p('Three'))), ol(li(p('One{>}')), li(p('Two'))))
        );
    ...
    describe('joining lists', () => {
      const expectedOutputForPreviousList = doc(
        ol(li(p('One')), li(p('Two')), li(p('Three')), li(p('Four')), li(p('Five'))),
        p('Six')
      );
      const expectedOutputForNextList = doc(
        p('One'),
        ol(li(p('Two')), li(p('Three')), li(p('Four')), li(p('Five')), li(p('Six')))
      );
      const expectedOutputForPreviousAndNextList = doc(
        ol(li(p('One')), li(p('Two')), li(p('Three')), li(p('Four')), li(p('Five')), li(p('Six')))
      );

      it("should join with previous list if it's of the same type", () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('One')), li(p('Two')), li(p('Three'))), p('{<}Four'), p('Five{>}'), p('Six'))
        );
      ...
      it("should join with next list if it's of the same type", () => {
        const { editorView, pluginState } = editor(
          doc(p('One'), p('{<}Two'), p('Three{>}'), ol(li(p('Four')), li(p('Five')), li(p('Six'))))
        );
    ...
    describe('Enter key-press', () => {
      ...
      context('when Enter key is pressed on non-empty nested list item', () => {
        it('should created new nested list item', () => {
          ...
          expect(editorView.state.doc).to.deep.equal(
            doc(ol(li(p('text'), ol(li(p('test')), li(p('{<>}')))), li(p('text'))))
          );
        });
      });
      ...
      context('when Enter key is pressed on non-empty top level list item inside panel', () => {
        it('should created new list item at top level', () => {
          ...
          expect(editorView.state.doc).to.deep.equal(
            doc(panel(ol(li(p('text')), li(p('test')), li(p('{<>}')), li(p('text')))))
          );
        });
      });
    ...
    describe('Toggle - nested list scenarios - to lift items out of list', () => {
      ...
      it('should be possible to toggle a selection across lists with different parent lists', () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('te{<}xt'), ol(li(p('text'))))), ol(li(p('te{>}xt'), ol(li(p('text'))))))
        );
      ...
      it('should be create a new list for children of lifted list item', () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('text'), ol(li(p('te{<>}xt'), ol(li(p('text')))))), li(p('text'))))
        );
      ...
      it('should only change type to bullet list when toggling orderedList to bulletList', () => {
        const { editorView, pluginState } = editor(
          doc(ol(li(p('text'), ol(li(p('text'), ol(li(p('te{<>}xt')))))), li(p('text'))))
        );
