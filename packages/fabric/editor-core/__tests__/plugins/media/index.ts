...
describe('Media plugin', () => {
  ...
  it('should invoke binary picker when calling insertFileFromDataUrl', async () => {
    ...
    const collectionFromProvider = jest.spyOn(pluginState, 'collectionFromProvider' as any);
    ...
    collectionFromProvider.mockRestore(); pluginState.destroy();
  });
  ...
  it('should remove failed uploads from the document', async () => {
    ...
    collectionFromProvider.mockRestore(); editorView.destroy(); pluginState.destroy();
  });
  ...
  it('should re-use old pickers when new media provider is set', async () => {
    ...
    expect(pickersAfterMediaProvider1).toHaveLength(pickersAfterMediaProvider2.length);
    for (let i = 0; i < pickersAfterMediaProvider1.length; i++) {
      expect(pickersAfterMediaProvider1[i]).toEqual(pickersAfterMediaProvider2[i]);
    }
  });
  ...
  it('should trigger analytics events for picking', async () => {
    ...
    expect(spy).toHaveBeenCalledWith('atlassian.editor.media.file.drop', { fileMimeType: 'file/test' });

    (pluginState as any).clipboardPicker!.handleUploadStart(testFileData);
    expect(spy).toHaveBeenCalledWith('atlassian.editor.media.file.paste', { fileMimeType: 'file/test' });

    (pluginState as any).popupPicker!.handleUploadStart(testFileData);
    expect(spy).toHaveBeenCalledWith('atlassian.editor.media.file.popup', { fileMimeType: 'file/test' });

    (pluginState as any).binaryPicker!.handleUploadStart(testFileData);
    expect(spy).toHaveBeenCalledWith('atlassian.editor.media.file.binary', { fileMimeType: 'file/test' });
  });
  ...
  describe('removeSelectedMediaNode', () => {
    ...
    describe('when selection is a non media node', () => {
      it('does not remove media node', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(hr, mediaGroup(deletingMediaNode)));
        editorView.destroy(); pluginState.destroy();
      });
    ...
    describe('when selection is text', () => {
      it('does not remove media node', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc('hello', mediaGroup(deletingMediaNode)));
        editorView.destroy(); pluginState.destroy();
      });
  ...
  it('should focus the editor after files are added to the document', async () => {
    ...
    spy.mockRestore(); editorView.destroy(); pluginState.destroy();
  });
  ...
  describe('Drop Placeholder', () => {
    ...
    it('should show the placeholder for code block', async () => {
      ...
      const dragZoneDom = getWidgetDom(editorView);
      expect(dragZoneDom).not.toBe(null);
      expect(dragZoneDom!.previousSibling!.textContent).toEqual('const foo = undefined;');
