  ...
  describe('Generic Picker', () => {
    ...
    describe('configures picker', () => {
      it('with correct upload params and context', () => {
        ...
        expect(mockPicker.moduleConfig).toHaveProperty('uploadParams', uploadParams);
        expect(mockPicker.moduleConfig).toHaveProperty('apiUrl', contextConfig.serviceHost);
        expect(mockPicker.moduleConfig).toHaveProperty('authProvider', contextConfig.authProvider);
      });
      ...
      it('respects dropzone component config', () => {
        ...
        expect(mockPicker.componentConfig).toHaveProperty('container', dropzoneContainer);
      });
      ...
      it('respects popup component config', () => {
        ...
        expect(mockPicker.componentConfig).toHaveProperty('userAuthProvider', contextConfig.userAuthProvider);
      });
    ...
    it('After upload has transitioned from "uploading", subsequent "status update" events must not downgrade status (ED-2062)', () => {
      ...
      expect(stateManager!.getState(testTemporaryFileId)!.status).toEqual('processing');
      ...
      expect(stateManager!.getState(testTemporaryFileId)!.status).toEqual('ready');
    });
  });
