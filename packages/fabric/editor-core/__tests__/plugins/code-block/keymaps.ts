...
import {
  doc, makeEditor, p, sendKeyToPm, code_block
...
describe('codeBlock - keymaps', () => {
  ...
  describe('Enter keypress', () => {
    describe('when enter key is pressed 2 times', () => {
      it('it should not exit code block', () => {
        ...
        sendKeyToPm(editorView, 'Enter');
        expect(editorView.state.doc).toEqualDocument(doc(code_block()('codeBlock\n\n')));
        editorView.destroy();
      });
    });
    ...
    describe('when enter key is pressed 3 times', () => {
      it('it should exit code block', () => {
        ...
        sendKeyToPm(editorView, 'Enter');
        sendKeyToPm(editorView, 'Enter');
        expect(editorView.state.doc).toEqualDocument(doc(code_block()('codeBlock'), p('{<>}')));
        editorView.destroy();
      });
    });
  });
  ...
  describe('Backspace', () => {
    ...
    it('should not remove the code block if selection is not empty ', () => {
      ...
      sendKeyToPm(editorView, 'Backspace');
      expect(editorView.state.doc).toEqualDocument(doc(code_block()('const x = 1;')));
      editorView.destroy();
    });
  });
});
