...
import {
  mention, em, strike, code, strong, insertText, doc, makeEditor, p, code_block, plain
...
describe('text-formatting input rules', () => {
  ...
  describe('strong rule', () => {
    it('should convert "**text**" to strong', () => {
      ...
      expect(trackEvent.calledWith('atlassian.editor.format.strong.autoformatting')).toBe(true);
    });
    ...
    it('should not convert the surrounding text to strong', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('hello', strong('text'), 'there')));
    });
    ...
    it('should not be inclusive right after autoformatting conversion', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(strong('text'), 'text')));
    });
    ...
    it('should convert "some**variables**" to strong', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('some', strong('variables'))));
    });
    ...
    it('should convert "hello __text__" to strong', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('hello ', strong('text'))));
    });
    ...
    it('should not convert "**text** to strong when node does not support text formatting', () => {
      ...
      expect(trackEvent.calledWith('atlassian.editor.format.strong.autoformatting')).toBe(false);
    });
  });
  ...
  describe('em rule', () => {
    it('should convert "*text*" to em', () => {
      ...
      expect(trackEvent.calledWith('atlassian.editor.format.em.autoformatting')).toBe(true);
    });
    ...
    it('should not be inclusive right after autoformatting conversion', () => {
      ...
      insertText(editorView, 'text', editorView.state.selection.$from.pos);
      expect(editorView.state.doc).toEqualDocument(doc(p(em('italic'), 'text')));
    });
    ...
    it('should keep current marks when converting from markdown', () => {
      ...
      insertText(editorView, '*italic*', sel);
      expect(editorView.state.doc).toEqualDocument(doc(p(strong('This is bold '), em(strong('italic')))));
    });
    ...
    it('should convert "some*variables*" to em', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('some', em('variables'))));
    });
    ...
    it('should convert "hello _text_" to em', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('hello ', em('text'))));
    });
  ...
  describe('strike rule', () => {
    it('should convert "~~text~~" to strike', () => {
      ...
      expect(trackEvent.calledWith('atlassian.editor.format.strike.autoformatting')).toBe(true);
    });
    ...
    it('should not be inclusive right after autoformatting conversion', () => {
      ...
      insertText(editorView, '~~text~~', sel);
      insertText(editorView, 'text', editorView.state.selection.$from.pos);
      expect(editorView.state.doc).toEqualDocument(doc(p(strike('text'), 'text')));
    });
    ...
    it('should convert "some~~texts~~" to strike', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('some', strike('texts'))));
    });
  ...
  describe('code rule', () => {
    ...
    it('should convert "some`texts`" to code', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('some', code('texts'))));
    });
    ...
    it('should convert mention to plaint text', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('hey! ', code('hello, @helga there'), '?')));
    });
    ...
    it('should cleanup other formatting', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(code('hello @helga, there?'))));
    });
    ...
    it('should not be inclusive right after autoformatting conversion', () => {
      ...
      insertText(editorView, '`text`', sel);
      insertText(editorView, 'text', editorView.state.selection.$from.pos);
      expect(editorView.state.doc).toEqualDocument(doc(p(code('text'), 'text')));
    });
  ...
  describe('nested rules', () => {
    ...
    it('should convert "*`text`*" to italic code text', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(em(code('text')))));
    });
    ...
    it('should convert "~~**text**~~" to strike strong', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('~~', strong('text'))));
      insertText(editorView, '~~', editorView.state.selection.from);
      expect(editorView.state.doc).toEqualDocument(doc(p(strike(strong('text')))));
      expect(trackEvent.calledWith('atlassian.editor.format.strong.autoformatting')).toBe(true);
      expect(trackEvent.calledWith('atlassian.editor.format.strike.autoformatting')).toBe(true);
    });
  });
