...
describe('hyperlink', () => {
  ...
  describe('API', () => {
    ...
    it('permits adding a link to an empty selection using the href', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href })(href))));
      editorView.destroy();
    });
    ...
    it('permits adding a link to an empty selection using the href and text', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href })(text))));
      editorView.destroy();
    });
    ...
    it('should add http:// for a link without protocol', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: hrefWithProtocol })(href))));
      editorView.destroy();
    });
    ...
    it('does not permit adding a link to an existing link', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://www.atlassian.com' })('link'))));
      editorView.destroy();
    });
    ...
    it('requires href when adding a link', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.com' })('text'))));
      editorView.destroy();
    });
    ...
    it('should not be a part of the link when typing before it', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(`abar`, link({ href })('text'))));
      editorView.destroy();
    });
    ...
    it('should be a part of the link when typing in it', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href })('tebarxt'))));
      editorView.destroy();
    });
    ...
    it('should not be a part of the link when typing after it', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href })('text'), 'bar')));
      editorView.destroy();
    });
    ...
    it('should add link in the correct position', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph('text'), paragraph(link({ href })('text'))));
      editorView.destroy();
    });
    ...
    describe('when a link is in the second paragraph', () => {
      it('should be able to unlink that link', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph('hello'), paragraph('text')));
        editorView.destroy();
      });
    });
    ...
    it('should be able to update existing links text', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://www.atlassian.com' })('Atlassian'))));
      editorView.destroy();
    });
    ...
    it('should escape from link mark when typing at the beginning of the link', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph('1', link({ href: 'http://example.com' })('text'))));
      editorView.destroy();
    });
    ...
    it('should not escape from link mark when typing at the middle of the link', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.com' })('t1ext'))));
      editorView.destroy();
    });
    ...
    describe('should update both href and text on edit if they were same before edit', () => {
      it('inserts a character inside a link', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.cxo' })('http://example.cxo'))));
        editorView.destroy();
      });
      ...
      it('inserts a character at the end of a link', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.com' })('http://example.com'), 'x')));
        editorView.destroy();
      });
      ...
      it('inserts a character at the beginning of a link', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph('x', link({ href: 'http://example.com' })('http://example.com'))));
        editorView.destroy();
      });
      ...
      it.skip('removes a character from the end of a link', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.co' })('http://example.co'))));
        editorView.destroy();
      });
      ...
      it('replaces a character inside a link', () => {
        ...
        sendKeyToPm(editorView, 'Backspace');
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://exampl.com' })('http://exampl.com'))));
        editorView.destroy();
      });
      ...
      it('replaces end of the link with extended content', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://example.co' })('http://example.co'), ' Atlassian')));
        editorView.destroy();
      });
      ...
      it('works with valid URLs without scheme', () => {
        ...
        expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://www.exampl.com' })('www.exampl.com'))));
        editorView.destroy();
      });
    });
  });
    ...
    describe('paste', () => {
      describe('url link is at beginning of plain text', () => {
        it('should add link mark', function () {
          ...
          expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://www.atlassian.com' })('http://www.atlassian.com'), ' test')));
          editorView.destroy();
        });
      });
      ...
      describe('url link has brackets', () => {
        it('should add link mark', function () {
          const { editorView } = editor(doc(paragraph('{<>}')));
          if (!dispatchPasteEvent(editorView, { plain: 'http://www.(atlassian).com test' })) {
            // This environment does not allow mocking paste events
      ...
      describe('url link is at end of html text', () => {
        it('should add link mark', function () {
          ...
          expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'http://www.atlassian.com' })('Atlassian'), ' test')));
          editorView.destroy();
        });
      });
      ...
      describe('url link without anchor tags in html in the middle of another text', () => {
        it('should add link mark', function() {
          const { editorView } = editor(doc(paragraph('{<>}')));
          ...
          expect(editorView.state.doc).toEqualDocument(doc(paragraph('testing ', link({ href: 'http://www.atlassian.com' })('Atlassian'), ' test')));
          editorView.destroy();
        });
      });
      ...
      describe('email link in the middle of plain text', () => {
        it('should add link mark', function () {
          const { editorView } = editor(doc(paragraph('{<>}')));
          if (!dispatchPasteEvent(editorView, { plain: 'test test@atlassian.com test' })) {
            return this.skip();
          }
          expect(editorView.state.doc).toEqualDocument(doc(paragraph('test ', link({ href: 'mailto:test@atlassian.com' })('test@atlassian.com'), ' test')));
          editorView.destroy();
        });
      });
      ...
      describe('email link without anchor tags in html', () => {
        it('should add link mark', function() {
          ...
          expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'mailto:test@atlassian.com' })('test@atlassian.com'), ' test')));
          editorView.destroy();
        });
      });

      describe('email link without anchor tags in html in the middle of another text', () => {
        it('should add link mark', function() {
          const { editorView } = editor(doc(paragraph('{<>}')));
      ...
      describe('email link is at end of html', () => {
        it('should add link mark', function () {
          ...
          expect(editorView.state.doc).toEqualDocument(doc(paragraph(link({ href: 'mailto:test@atlassian.com' })('Atlassian'), ' test')));
          editorView.destroy();
        });
      });
      ...
      describe('email link with inline style is pasted and then text is inserted', () => {
        it('should not apply inline style to inserted text', function() {
          ...
          expect(editorView.state.doc).toEqualDocument(
            doc(paragraph(link({ href: 'mailto:test@atlassian.com' })(strong('Atlassian')), 'abc'))
          );
          editorView.destroy();
        });
      });
  ...
  describe('Message Appearance', () => {
    ...
    it('should remove link mark if visible text is not a valid link - 2', () => {
      ...
      sendKeyToPm(editorView, 'Backspace');
      expect(editorView.state.doc).toEqualDocument(doc(paragraph('www.atlassian.c')));
    });
