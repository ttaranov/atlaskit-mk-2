...
describe('hyperlink', () => {
  ...
  describe('input rules', () => {
    ...
    it('should not convert "https://www.atlassian.com" to hyperlink inside a code_block', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(code_block()('https://www.atlassian.com ')));
    });
    ...
    it('should not convert "javascript://alert(1) " to hyperlink', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('javascript://alert(1); ')));
    });
    ...
    it('should convert prettyandsimple@example.com to a link', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(
        doc(p(link({ href: 'mailto:prettyandsimple@example.com' })('prettyandsimple@example.com'), ' '))
      );
    });
    ...
    it('should not convert invalid emails like to a mailto link (double dot)', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('john.doe@example..com ')));
    });
    ...
    it('should convert "[text](http://foo)" to hyperlink', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(link({ href: 'http://foo' })('text'))));
    });
    ...
    it('should convert text with spaces "[text text](http://foo)" to hyperlink', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(link({ href: 'http://foo' })('text text'))));
    });
    ...
    it('should convert "[text](http://foo)" to hyperlink inside a code_block', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(code_block()('[text](http://foo)')));
    });
    ...
    it('converts to hyperlink if possible hyperink text is after a new line and previous line has an hyperlink', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p(firstLink, br, p(secondLink, ' '))));
    });
