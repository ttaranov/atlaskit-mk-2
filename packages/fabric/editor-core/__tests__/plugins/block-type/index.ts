...
describe('block-type', () => {
  ...
  describe('code block', () => {
    it('should be able to insert code block', () => {
      ...
      expect(editorView.state.doc).toEqualDocument(doc(p('te'), code_block()(), p('xt')));
      editorView.destroy();
    });
  });
  ...
  it('should have all of the present blocks type panel, blockQuote, codeBlock in availableWrapperBlockTypes', () => {
    ...
    expect(pluginState.availableWrapperBlockTypes.some(blockType => blockType.name === 'panel')).toBe(true);
    expect(pluginState.availableWrapperBlockTypes.some(blockType => blockType.name === 'codeblock')).toBe(true);
    expect(pluginState.availableWrapperBlockTypes.some(blockType => blockType.name === 'blockquote')).toBe(true);
  });
  ...
  it('should be able to change multiple paragraphs into one blockquote', () => {
    ...
    pluginState.insertBlockType('blockquote', editorView);
    expect(editorView.state.doc).toEqualDocument(doc(blockquote(p('li{<}ne1'), p('li{>}ne2'), p('li{>}ne3'))));
    editorView.destroy();
  });
  ...
  describe('insertBlockType', () => {
    ...
    it('should be able to insert panel after current selection if current selection can not be wrapper in panel', () => {
      ...
      pluginState.insertBlockType('panel', editorView);
      expect(editorView.state.doc).toEqualDocument(doc(blockquote('test'), panel(p())));
      editorView.destroy();
    });
    ...
    it('should be able to insert blockquote after current selection if current selection can not be wrapper in blockquote', () => {
      const { pluginState, editorView } = editor(doc(h1('test{<>}')));
      pluginState.insertBlockType('blockquote', editorView);
      expect(editorView.state.doc).toEqualDocument(doc(h1('test'), blockquote(p())));
      editorView.destroy();
    });
    ...
    it('should insert code block after selection if selected block has text', () => {
      const { pluginState, editorView } = editor(doc(p('text{<>}')));
      pluginState.insertBlockType('codeblock', editorView);
      expect(editorView.state.doc).toEqualDocument(doc(p('text{<>}'), code_block()()));
      editorView.destroy();
    });
  });
