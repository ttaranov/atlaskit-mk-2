...
describe(name, () => {
  describe('EditorServicesConfig interface', () => {
    ...
    it('should compile with TypeScript.', () => {
      ...
      expect(configInstance.emojiResourceProvider!() instanceof Promise).toBe(true);
      expect(configInstance.mediaResourceProvider!() instanceof Promise).toBe(true);
      expect(configInstance.mentionResourceProvider!() instanceof Promise).toBe(true);
