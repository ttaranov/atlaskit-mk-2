...
const checkBuilder = (fn: any, description: string, cxhtml: string, doc: PMNode) => {
  fn(`parses CXHTML: ${description}`, () => {
    ...
    expect(actual).toEqualDocument(doc);
  });

  fn(`round-trips CXHTML: ${description}`, () => {
    const roundTripped = parse(encode(doc));
    expect(roundTripped).toEqualDocument(doc);
  });
};
...
describe('ConfluenceTransformer: encode - parse:', () => {
  ...
  describe('basic formatting:', () => {
    ...
    describe('marks formatting:', () => {
      ...
      it('parses text color in hex', () => {
        ...
        expect(actual).toEqualDocument(doc(p(
          'Text with ',
  ...
  +// bar!]]></ac:plain-text-body></ac:structured-macro>`);
        expect(actual).toEqualDocument(doc(codeblock({ language: null })(`foo
  +// bar!`)));
      });
      ...
      describe('when pasted from Editor', () => {
        it('block wysiwyg-macro noformat: parses CXHTML to codeBlock', () => {
          ...
          expect(actual).toEqualDocument(doc(codeblock({ language: null })('No format content')));
        });
      });
  ...
  describe('ac:emoticon', () => {
    ...
    it('valid emoji with all fabric emoji attributes encoded with ac:name="blue-star"', ()=>{
      ...
      expect(emoticon).toContain('ac:name="blue-star"');
    });
    ...
    it('hipchat-emoticons encoded with ac:name="blue-star" and preserve original ac:shortcut as ac:emoji-shortname', () => {
      ...
      expect(parse(notMappedEmoticon)).toEqual(parse(encodedEmoticon));

      const notMappedEmoticonEncode = encode(parse(notMappedEmoticon));
      expect(notMappedEmoticonEncode).toContain('ac:name="blue-star"');
      expect(notMappedEmoticonEncode).toContain('ac:emoji-id="atlassian-anything-not-supported"');
      expect(notMappedEmoticonEncode).toContain('ac:emoji-shortname=":anything-not-supported:"');
    });
  });
  ...
  describe('fabric mentions', () => {
    ...
    it('strips @ from mention text', () => {
      ...
      expect(parse(encode(docBefore))).toEqual(docAfter);
    });
  });
  ...
  describe('media nodes', () => {
    ...
    it('should encode/parse media nodes with own attributes', () => {
      ...
      expect(parsed).toEqualDocument(docNode);
      expect(parse(encode(docNode))).toEqualDocument(docNode);

      // check that node attributes are set during parsing
      const parsedMediaNode = parsed.firstChild!.firstChild;
      expect(parsedMediaNode!.attrs.__fileName).toEqual('2017-04-12 07.15.57.jpg');
      expect(parsedMediaNode!.attrs.__fileSize).toEqual(95316);
      expect(parsedMediaNode!.attrs.__fileMimeType).toEqual('image/jpeg');
    });
    ...
    it('should put media and paragraph text into different block nodes (simple case)', () => {
      ...
      expect(docNode).toEqualDocument(parseWrap());
    });
  });
});
