...
describe('BitbucketTransformer: serializer', () => {
  ...
  it(
    'should escape 4 spaces at the beginning of the line with zero-width-non-joiner',
    () => {
      const fourSpaces = '    ';
  ...
  it(
    'should preserve leading and trailing blank lines using zero-non-width',
    () => {
      expect(markdownSerializer.serialize(doc(
        p(),
        p('bar')
  ...
  describe('mentions', () => {
    ...
    it(
      'should not add a blank space in the end of the string for mentions',
      () => {
    ...
    it(
      'should not divide mention and text with additional space if text starts with the space',
      () => {
        const node = doc(p(mention({ text: 'Oscar Wallhult', id: 'oscar' }), ' text'));
        const test = markdownSerializer.serialize(node);
        expect(test).toEqual('@oscar text');
    ...
    it(
      'should divide mention and text with only one additional space if text starts with the spaces',
      () => {
        const node = doc(p(mention({ text: 'Oscar Wallhult', id: 'oscar' }), '  text'));
        const test = markdownSerializer.serialize(node);
        expect(test).toEqual('@oscar  text');
    ...
    it(
      'should not divide mention and italic text node with additional space if text starts with the space',
      () => {
  ...
  it('should serialize headings (level 1 - 6)', () => {
    expect(markdownSerializer.serialize(doc(h1('level 1')))).toEqual('# level 1');
    expect(markdownSerializer.serialize(doc(h2('level 2')))).toEqual('## level 2');
    expect(markdownSerializer.serialize(doc(h3('level 3')))).toEqual('### level 3');
    expect(markdownSerializer.serialize(doc(h4('level 4')))).toEqual('#### level 4');
    expect(markdownSerializer.serialize(doc(h5('level 5')))).toEqual('##### level 5');
    expect(markdownSerializer.serialize(doc(h6('level 6')))).toEqual('###### level 6');
  ...
  describe('ordered list', () => {
    ...
    it('with one empty element is preserved', () => {
      expect(markdownSerializer.serialize(doc(ol(li(p('')))))).toEqual('1. \n\n');
    });
  ...
  describe('image', () => {
    ...
    it(
      'with title containing double-quote to serialize without escaping',
      () => {
        expect(markdownSerializer.serialize(doc(img({
          src: 'http://example.com',
          alt: 'an image',
  ...
  describe('blockquotes', () => {
    it('should serialized', () => {
      expect(markdownSerializer.serialize(doc(blockquote(p('foo'))))).toEqual('> foo');
      expect(markdownSerializer.serialize(doc(
        blockquote(p('foo')),
        blockquote(p('bar')),
      ))).toEqual('> foo\n\n> bar');
  ...
  describe('marks -', () => {
    it('should ignore mentionQuery mark', () => {
      expect(markdownSerializer.serialize(doc(p(mentionQuery('@oscar'))))).toEqual('@oscar');
    });

    it('should ignore emojiQuery mark', () => {
      expect(markdownSerializer.serialize(doc(p(emojiQuery(':grin'))))).toEqual(':grin');
    });
    ...
    it('should serialize strong', () => {
      expect(markdownSerializer.serialize(doc(p(strong('foo'))))).toEqual('**foo**');
      expect(markdownSerializer.serialize(doc(p(
        'foo ',
        strong('bar bar'),
        ' baz',
    ...
    it('should serialize strikethrough', () => {
      expect(markdownSerializer.serialize(doc(p(strike('foo'))))).toEqual('~~foo~~');
      expect(markdownSerializer.serialize(doc(p(
        'foo ',
        strike('bar bar'),
        ' baz',
    ...
    it('should serialize code', () => {
      expect(markdownSerializer.serialize(doc(p(code('foo'))))).toEqual('`foo`');
      expect(markdownSerializer.serialize(doc(p(
        'foo ',
        code('bar baz'),
        ' foo',
    ...
    describe('code', () => {
      ...
      it(
        'containing backticks on the edges of a fence should be fenced properly',
        () => {
          expect(markdownSerializer.serialize(doc(p(
            'foo ',
            code('`bar`  ``baz``'),
    ...
    xdescribe('links', () => {
      ...
      it('with title to serialize', () => {
        ...
        expect(markdownSerializer.serialize(doc(p(
          link('foo'),
        )))).toEqual('[foo](http://example.com "title")');
      });
      ...
      it('with title containing double-quotes to serialize', () => {
        ...
        expect(markdownSerializer.serialize(doc(p(
          link('foo'),
        )))).toEqual('[foo](http://example.com "some " "title"")');
      });
      ...
      it('with space in url and title to serialize', () => {
        ...
        expect(markdownSerializer.serialize(doc(p(
          link('foo'),
        )))).toEqual('[foo](/url with space "title")');
      });
