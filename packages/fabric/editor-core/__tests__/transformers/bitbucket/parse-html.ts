...
describe('BitbucketTransformer: parser', () => {
  describe('block elements', () => {
    ...
    it('should remove all zero-with-non-joiners', () => {
      expect(parse('<p>foo</p><p>&zwnj;</p><p>&zwnj;</p><p>bar</p>')).toEqualDocument(doc(p('foo'), p(''), p(''), p('bar')));
    });
    ...
    it('should support images', () => {
      ...
      expect(parsed).toEqualDocument(doc(p(img({ src: 'http://path/to/image.jpg', alt: 'Alt text' }))));
    });
  });
  ...
  describe('inline elements', () => {
    ...
    it('should support code', () => {
      ...
      expect(textWithMarks(parse('<p><span style="font-family: monospace;">text</span></p>'), 'text', [ code ])).toBe(true);
    });

    it('should support links', () => {
      const link = schema.marks.link.create({ href: 'http://example.com' });
      expect(textWithMarks(parse('<p><a href="http://example.com">example link</a></p>'), 'example link', [ link ])).toBe(true);
    });

    it('should support both strong and em', () => {
      const em = schema.marks.em.create();
      const strong = schema.marks.strong.create();
      expect(textWithMarks(parse('<p><strong><em>text</em></strong></p>'), 'text', [em, strong])).toBe(true);
    });
  });

  describe('blockquotes', () => {
    it('should be parsed', () => {
      expect(parse('<blockquote><p>text</p></blockquote>')).toEqualDocument(doc(blockquote(p('text'))));
    });
  });
  ...
  describe('tables', () => {
    ...
    it('with a single column should be converted into table', () => {
      ...
      expect(result).toEqualDocument(doc(
        table(
