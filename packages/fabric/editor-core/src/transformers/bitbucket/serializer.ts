...
const editorNodes = {
  ...
  image(state: MarkdownSerializerState, node: PMNode) {
    ...
    state.write(
      '![' +
        escapeMarkdownForImageHotfix(node.attrs.alt) +
        '](' +
        escapeMarkdownForImageHotfix(node.attrs.src) +
        (node.attrs.title
          ? ` '${escapeMarkdownForImageHotfix(node.attrs.title)}'`
          : '') +
        ')'
    );
  },
