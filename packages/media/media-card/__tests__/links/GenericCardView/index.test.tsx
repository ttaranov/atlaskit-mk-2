...
describe('LinkCardGenericView', () => {
  ...
  it('should render the context text when I have a site and url', () => {
    ...
    expect(element.find(BlockResolvedView).prop('context')).toEqual(
  ...
  it('should render a title when there is a title', () => {
    ...
    expect(element.find(BlockResolvedView).prop('title')).toEqual({
      text: 'foobar',
    });
  });
  ...
  it('should not render a title when there is no title', () => {
    const element = shallow(<LinkCardGenericView />);
    expect(element.find(BlockResolvedView).prop('title')).toBeUndefined();
  });

  it('should render a description when there is a description', () => {
    const element = shallow(<LinkCardGenericView description="foobar" />);
    expect(element.find(BlockResolvedView).prop('description')).toEqual({
      text: 'foobar',
    });
  });
  ...
  it('should not render a description when there is no description', () => {
    const element = shallow(<LinkCardGenericView />);
    expect(element.find(BlockResolvedView).prop('description')).toBeUndefined();
  });
  ...
  it('should render a preview when the card is square and we have a thumbnailUrl', () => {
    ...
    expect(element.find(BlockResolvedView).prop('preview')).toEqual(
      thumbnailUrl,
    );
    expect(element.find(BlockResolvedView).prop('thumbnail')).toBeUndefined();
  });
  ...
  it('should render a thumbnail when the card is horizontal and we have a thumbnailUrl', () => {
    ...
    expect(element.find(BlockResolvedView).prop('preview')).toBeUndefined();
    expect(element.find(BlockResolvedView).prop('thumbnail')).toEqual(
      thumbnailUrl,
    );
  });
});
