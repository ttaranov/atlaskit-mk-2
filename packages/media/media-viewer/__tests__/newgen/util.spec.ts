import { addAuthTokenToUrl } from '../../src/newgen/util';
...
describe('addAuthTokenToUrl', () => {
  it('should add auth token and client query parameters to the url when auth is client based', async () => {
    ...
    const clientAuthProvider = jest.fn(() => Promise.resolve({ token, clientId }));
    const context = createContext(clientAuthProvider);
    const url = await addAuthTokenToUrl('/file/3333-4444-5555', context, 'mycollection');
    expect(url).toEqual('some-service-host/file/3333-4444-5555?client=some-client-id&collection=mycollection&token=some-token');
  });
  ...
  it('should add the auth token to the url when auth type is ASAP', async () => {
    ...
    const url = await addAuthTokenToUrl('/file/3333-4444-5555', context, 'mycollection');
    expect(url).toEqual('some-service-host/file/3333-4444-5555?collection=mycollection&token=some-token');
  });
